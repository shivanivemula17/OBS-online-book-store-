...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if usertype == 'Customer':
            ...
            new_customer = users.objects.create(UserType=usertype,FirstName=firstname,LastName=lastname,Username=email_customer,Password=password,Address=address,Mobile_No=mobile_no,Zipcode=zip_code)
            new_customer.save()
            ...
            if password != confirm_password:
                messages.error(request, "Passwords do not match.")
                return redirect('register') 
            new_vendor = users.objects.create(UserType=usertype,CompanyName=company_name,CompanyLocation=company_location,GST_No=gstno,Username=email_vendor,Password=password,Mobile_No=mobile_no,Zipcode=zip_code)
            new_vendor.save()
